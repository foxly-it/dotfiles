...
# ===================== Konfiguration / CLI =====================
REPO_URL="${1:-}"                # optional: ./install.sh https://github.com/foxly-it/dotfiles.git
BRANCH="${BRANCH:-main}"         # Branch fürs Clone (nur bei REPO_URL)
...
need_apt run-parts || true  # meist in coreutils, harmless

# lolcat (systemweit)
if ! command -v /usr/local/bin/lolcat >/dev/null 2>&1; then
  msg "  • lolcat über RubyGems…"
...
if [[ -f "$LOCAL_HEADER" && -f "$LOCAL_SYSINFO" ]]; then
  ...
  [[ -z "$REPO_URL" ]] && { err "Keine Repo-URL. Abbruch."; exit 1; }
  WORKDIR="$(mktemp -d)"
  ...
  [[ -z "$SRC_HEADER" ]] && { err "update-motd.d/{00-header,10-sysinfo} nicht im Repo gefunden."; exit 1; }
fi
...
if command -v run-parts >/dev/null 2>&1; then
  ...
  # Fallback: alle ausführbaren Skripte manuell laufen lassen
  {
...
exit 0
EOF
sudo install -m 0755 -o root -g root "$TEMP_GEN" /usr/local/sbin/generate-motd
rm -f "$TEMP_GEN"
ok "Generator bereit."

# ===================== 5) PAM: bei Login generieren & anzeigen =
msg "[5/7] PAM/MOTD in /etc/pam.d/sshd konfigurieren…"
PAM_FILE="/etc/pam.d/sshd"
sudo cp -a "$PAM_FILE" "$PAM_FILE.bak.$(date +%Y%m%d%H%M%S)"

# a) pam_exec zum Generieren VOR den pam_motd-Zeilen einfügen (falls nicht vorhanden)
if ! sudo grep -q 'pam_exec\.so.*generate-motd' "$PAM_FILE"; then
  sudo awk '
    BEGIN{ins="session optional pam_exec.so stdout /usr/local/sbin/generate-motd"; done=0}
    /pam_motd\.so/ && !done {print ins; done=1}
    {print}
...
# b) sicherstellen, dass /run/motd.dynamic und /etc/motd angezeigt werden
sudo grep -q '/run/motd.dynamic' "$PAM_FILE" || echo 'session optional pam_motd.so motd=/run/motd.dynamic' | sudo tee -a "$PAM_FILE" >/dev/null
sudo grep -q 'pam_motd\.so noupdate' "$PAM_FILE" || echo 'session optional pam_motd.so noupdate' | sudo tee -a "$PAM_FILE" >/dev/null
ok "PAM/MOTD konfiguriert. Backup: ${PAM_FILE}.bak.*"

# ===================== 6) sshd_config nachfragen & setzen ======
msg "[6/7] sshd_config prüfen…"
SSHD="/etc/ssh/sshd_config"
SET_IT=false
sudo grep -Eq '^\s*UsePAM\s+yes\b' "$SSHD" || SET_IT=true
sudo grep -Eq '^\s*PrintMotd\s+no\b' "$SSHD" || SET_IT=true
...
find /etc/update-motd.d/ -type f ! -name '00-header' ! -name '10-sysinfo' -exec rm -f {} +
# ===================== 7) optional Ubuntu-Defaults ausblenden ==
...
# einmal generieren und anzeigen
sudo /usr/local/sbin/generate-motd || true
echo "----- MOTD (Preview) -----"
sudo cat /run/motd.dynamic || true
echo "--------------------------"

ok "Fertig! Bitte neu per SSH einloggen, um die MOTD zu sehen."
